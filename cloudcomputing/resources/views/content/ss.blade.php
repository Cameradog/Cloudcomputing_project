...
<div class="container-fluid move-down">
  ...
  @foreach($topTenData as $key => $value)
    <input id="topChartData{{ $key+1 }}" name="topChartData{{ $key+1 }}" type="hidden" value="{{ $value->ss }}" data-word="{{ $value->words }}" data-token="{{ csrf_token() }}">
  @endforeach
    ...
    <table class="table table-hover table-striped">
      <thead>
        <tr>
          ...
          <th>Science</th>
          <th  class="danger">SocialScience</th>
          <th></th>
        </tr>
      </thead>
      <tbody>
