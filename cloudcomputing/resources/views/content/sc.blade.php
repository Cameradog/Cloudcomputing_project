...
<div class="container-fluid move-down">
  ...
  @foreach($topTenData as $key => $value)
    <input id="topChartData{{ $key+1 }}" name="topChartData{{ $key+1 }}" type="hidden" value="{{ $value->sc }}" data-word="{{ $value->words }}" data-token="{{ csrf_token() }}">
  @endforeach
